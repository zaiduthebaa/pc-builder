...
const ShowAccessories = () => {
    ...
    for (let item of localData) {
      ...
      if (recommendedAccessories.length >= 8) {
        break;
      }
    }
  ...
  const ShowSimilarProducts = () => {
    // Initialize an array for recommended products
    const recommendedProducts = [];

    // Filter products within the price range and exclude the current product
    for (let item of localData) {
      if (item.id !== product.id) {
        recommendedProducts.push(item);
      }

      // If we have 8 recommendations, stop filtering
